...
impl Floor {
    pub fn new(input: String) -> Self {
        Self {
            _rock_index: 0,
            _directions: input.chars().collect_vec(),
            _jet_direction_index: 0,
    ...
    pub fn next_rock(&mut self, y_min: usize) -> RockEnum {
        let result = RockEnum::new(self._rock_index, 2, y_min);
        self._rock_index = (self._rock_index + 1) % ROCK_AMOUNT;
        result
    }
    ...
    pub fn next_jet_direction(&mut self) -> char {
        let result = self._directions[self._jet_direction_index];
        self._jet_direction_index = (self._jet_direction_index + 1) % self._directions.len();
        result
    }

    pub fn get_jet_direction_index(&self) -> usize {
        self._jet_direction_index
    }
...
impl Floor {
    ...
    fn follow_pattern(
        ...
        new_total_rock_index = new_total_rock_index + amount_of_full_cycles * diff_rock_index;
        new_y_max = new_y_max + amount_of_full_cycles * diff_y;
